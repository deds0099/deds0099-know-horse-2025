...
const AdminSubscriptions = () => {
  ...
  const [minicourseFilter, setMinicourseFilter] = useState<string>('all');
  ...
  const handlePaymentStatusChange = async (id: string, isPaid: boolean, type?: 'subscription' | 'minicourse') => {
    try {
      console.log('Iniciando atualização manual de pagamento:', { id, isPaid, type });

      await updatePaymentStatus(id, isPaid, type);

      toast.success(`Status de pagamento atualizado com sucesso`);
      await refetch(); // Atualizar dados usando useQuery
    } catch (error: any) {
      console.error('Erro ao atualizar status de pagamento:', error);
  ...
  // Obter lista única de minicursos para o filtro
  const minicourseTitles = Array.from(new Set(
    subscriptions
      .filter(sub => sub.type === 'minicourse' && sub.minicourse_title)
      .map(sub => sub.minicourse_title!)
  )).sort();
  ...
  const filteredSubscriptions = [...subscriptions]
    .filter(sub => {
      ...
      if (statusFilter === 'pending' && sub.is_paid) return false;

      // Filtro de minicurso
      if (minicourseFilter !== 'all') {
        if (sub.type !== 'minicourse' || sub.minicourse_title !== minicourseFilter) {
          return false;
        }
      }
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Gerencie todas as inscrições para o Congresso Equestre 2026
              </p>
            </div>
          ...
          <Card className="backdrop-blur-sm bg-white/70 border border-white/20 mb-8">
            <CardHeader>
              ...
              <CardDescription>
                Gerencie as inscrições do Congresso Equestre 2026
                <br />
              ...
              <div className="mb-6 space-y-4">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="w-full md:w-64">
                    <Select value={minicourseFilter} onValueChange={setMinicourseFilter}>
                      <SelectTrigger>
                        <SelectValue placeholder="Filtrar por Minicurso" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">Todos os Minicursos</SelectItem>
                        {minicourseTitles.map(title => (
                          ...
                          <td className="p-2">
                            <div className="flex space-x-2">
                              <Button
                                ...
                                onClick={() => handlePaymentStatusChange(item.id, !item.is_paid, item.type)}
